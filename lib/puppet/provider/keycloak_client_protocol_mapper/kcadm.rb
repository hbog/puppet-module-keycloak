...
Puppet::Type.type(:keycloak_client_protocol_mapper).provide(:kcadm, parent: Puppet::Provider::KeycloakAPI) do
      ...
      clients_data.each do |client|
        ...
        data.each do |d|
          ...
          if ['oidc-usermodel-client-role-mapper', 'oidc-usermodel-property-mapper',
              'oidc-group-membership-mapper','oidc-usermodel-realm-role-mapper',].include?(protocol_mapper[:type]) || (protocol_mapper[:protocol] == 'openid-connect' && protocol_mapper[:type] =~ %r{script-.+})
  ...
  def create
    ...
    if ['oidc-usermodel-client-role-mapper', 'oidc-usermodel-property-mapper',
        'oidc-group-membership-mapper','oidc-usermodel-realm-role-mapper',].include?(resource[:type]) || (resource[:protocol] == 'openid-connect' && resource[:type] =~ %r{script-.+})
